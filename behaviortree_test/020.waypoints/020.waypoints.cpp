...
class GenerateWaypoints : public SyncActionNode {
   public:
    ...
    static PortsList providedPorts() { return {OutputPort<SharedQueue<Pose2D>>("waypoints")}; }
};
//--------------------------------------------------------------
class PrintNumber : public SyncActionNode {
   public:
    ...
    static PortsList providedPorts() { return {InputPort<double>("value")}; }
};
...
class UseWaypoint : public ThreadedAction {
   public:
    UseWaypoint(const std::string& name, const NodeConfig& config) : ThreadedAction(name, config) {}
    ...
    static PortsList providedPorts() { return {InputPort<Pose2D>("waypoint")}; }
};
