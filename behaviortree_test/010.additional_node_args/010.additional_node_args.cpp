...
class Action_A : public SyncActionNode {
   public:
    ...
    NodeStatus tick() override {
        std::cout << name() << ": " << _arg1 << " / " << _arg2 << " / " << _nc.value() << std::endl;
...
class Action_B : public SyncActionNode {
   public:
    Action_B(const std::string& name, const NodeConfig& config) : SyncActionNode(name, config) {}
...
int main() {
    ...
    factory.registerNodeType<Action_A>("Action_A", 42, "hello world", std::ref(non_copyable));
